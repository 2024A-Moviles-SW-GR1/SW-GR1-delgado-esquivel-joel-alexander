...
class ESqliteActorHelper(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, 1) {
    ...
    override fun onCreate(db: SQLiteDatabase?) {
        ...
        val createTableQuery =
            "CREATE TABLE $NOMBRE_TABLA_ACTOR (${COLUMNA_ACTOR_ID} INTEGER PRIMARY KEY AUTOINCREMENT, $COLUMNA_NOMBRE TEXT, $COLUMNA_APELLIDO TEXT, $COLUMNA_EDAD INTEGER, $COLUMNA_NACIONALIDAD TEXT, $COLUMNA_ACTIVO BOOLEAN, $COLUMNA_LATITUD DOUBLE, $COLUMNA_LONGITUD DOUBLE)"
        db?.execSQL(createTableQuery)
        Log.d("Database", "Table ACTOR created")

        //Crear la tabla MOVIE
        val createTableQuery2 =
            "CREATE TABLE $NOMBRE_TABLA_MOVIE ($COLUMNA_MOVIE_ID INTEGER PRIMARY KEY AUTOINCREMENT, $COLUMNA_TITULO TEXT, $COLUMNA_DIRECTOR TEXT, $COLUMNA_GENERO TEXT, $COLUMNA_ANIO INTEGER, $COLUMNA_SINOPSIS TEXT, $COLUMNA_ID_ACTOR INTEGER, FOREIGN KEY($COLUMNA_ID_ACTOR) REFERENCES ACTOR(id) ON DELETE CASCADE)"
        ...
        if (existeAlMenosUna) {
            do {
                val actor = Actor(
                    ...
                    resultadoConsultaLectura.getDouble(
                        resultadoConsultaLectura.getColumnIndexOrThrow(
                            COLUMNA_LATITUD
                        )
                    ),
    ...
    fun consultarActorPorId(id: Int): Actor? {
        ...
        val latitud = resultadoConsultaLectura.getDouble(
            resultadoConsultaLectura.getColumnIndexOrThrow(COLUMNA_LATITUD)
        )
        val longitud = resultadoConsultaLectura.getDouble(
            resultadoConsultaLectura.getColumnIndexOrThrow(COLUMNA_LONGITUD)
        )

        baseDatosLectura.close()
        resultadoConsultaLectura.close()
        return Actor(id, name, lastName, age, nationality, active, latitud, longitud)
    }
